...
func statesEndpoint(api huma.API, session bluetooth.Session) {
	type AdapterStatesInput struct {
		Powered      string `query:"set_powered" enum:"enable,disable" doc:"Set the adapter's powered state."`
		Pairable     string `query:"set_pairable" enum:"enable,disable" doc:"Set the adapter's pairable state."`
		Discoverable string `query:"set_discoverable" enum:"enable,disable" doc:"Set the adapter's discoverable state."`
		Discovery    string `query:"set_discovery" enum:"enable,disable" doc:"Toggle the adapter's device discovery mode."`
	}

	type AdapterStatesOutput struct {
		Body struct {
			PoweredState      string `json:"powered_state,omitempty" enum:"enabled,disabled" doc:"The adapter's powered state."`
			PairableState     string `json:"pairable_state,omitempty" enum:"enabled,disabled" doc:"The adapter's pairable state."`
			DiscoverableState string `json:"discoverable_state,omitempty" enum:"enabled,disabled" doc:"The adapter's discoverable state."`
			DiscoveryState    string `json:"discovery_state,omitempty" enum:"enabled,disabled" doc:"The adapter's device discovery mode state."`
		}
	}
	...
	huma.Register(api, huma.Operation{
		...
		Summary:     "States",
		Description: "This endpoint, when called by itself, fetches the different states (powered, pairable, discoverable and device discovery) of an adapter. Use the **query parameters** to `enable` or `disable` each state.",
		Tags:        []string{"Adapter"},
	}, func(_ context.Context, input *struct {
		AdapterStatesInput
		AddressInput
